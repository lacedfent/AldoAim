...
--// --- Gun Mods Logic ---
--// ===================================================================================
local WeaponsFolder = ReplicatedStorage:WaitForChild("Weapons", 10) -- Wait up to 10s for the folder
...
--// Backs up the original stats of a weapon
...
--// Applies the currently selected mods to a single weapon
...
--// Loops through all known weapons and applies mods
...
--// Restores all weapons to their original backed-up state
...
--// Initial setup for Gun Mods
if WeaponsFolder then
	-- Backup existing weapons
	for _, weapon in ipairs(WeaponsFolder:GetChildren()) do
		backupSingleWeapon(weapon)
	end
	-- Listen for new weapons being added during gameplay
	WeaponsFolder.ChildAdded:Connect(function(weapon)
        task.wait() -- Allow weapon properties to load in
        backupSingleWeapon(weapon)
        applyGunModsToWeapon(weapon)
    end)
end
...
local function isValidTarget(model)
	...
	else -- If it's not a player, it's an NPC, and we always target them now
    end
	return true
end
...
local function aimAt(model)
	...
	elseif settings.aimMethod == "Mouse" then
		if mousemoverel then
			local targetPos, onScreen = screenPoint(targetPart.Position)
			if onScreen then
...
local guiElements = {}
local mainGuiFrame -- Declare here to be accessible by the input connection
...
rsConn = RunService.RenderStepped:Connect(function(delta)
	...
	if settings.aimbotEnabled and isAimbotKeyPressed and targetModel then aimAt(targetModel) end
...
local title = Instance.new("TextLabel"); title.BackgroundTransparency = 1; title.Position = UDim2.new(0, 12, 0, 0); title.Size = UDim2.new(1, -200, 1, 0); title.Text = "AldoAimV3.1 | Arsenal"; title.TextXAlignment = Enum.TextXAlignment.Left; title.TextColor3 = Color3.fromRGB(255, 255, 255); title.Font = Enum.Font.GothamBold; title.TextSize = 12; title.Parent = titleBar
...
tabAimbotBtn.Size = UDim2.new(0, buttonWidth, 0, buttonHeight)
tabAimbotBtn.Position = UDim2.new(0.5, -buttonWidth / 2, 0, buttonSpacing)

tabESPBtn.Size = UDim2.new(0, buttonWidth, 0, buttonHeight)
tabESPBtn.Position = UDim2.new(0.5, -buttonWidth / 2, 0, buttonSpacing * 2 + buttonHeight)

tabGunModsBtn.Size = UDim2.new(0, buttonWidth, 0, buttonHeight)
tabGunModsBtn.Position = UDim2.new(0.5, -buttonWidth / 2, 0, buttonSpacing * 3 + buttonHeight * 2)

tabMiscBtn.Size = UDim2.new(0, buttonWidth, 0, buttonHeight)
tabMiscBtn.Position = UDim2.new(0.5, -buttonWidth / 2, 0, buttonSpacing * 4 + buttonHeight * 3)

tabSettingsBtn.Size = UDim2.new(0, buttonWidth, 0, buttonHeight)
tabSettingsBtn.Position = UDim2.new(0.5, -buttonWidth/2, 1, -(buttonHeight + buttonSpacing))
...
btnClose.MouseButton1Click:Connect(function() if rsConn then rsConn:Disconnect() end; restoreAllWeaponData(); setNoclipEnabled(false); setFullBright(false); pcall(function() Camera.FieldOfView = _G.AimESP_Originals.Fov end); for m,_ in pairs(ESP.cache) do removeCache(m) end; pcall(function() fovCircle:Remove() end); sg:Destroy() end)
...
guiElements.aimRadius = mkSlider(pageAim, "FOV Radius", 50, 600, 1, function() return settings.aimRadius end, function(v) settings.aimRadius = v end, 134)
...
--// Populate Gun Mods Page
mkToggle(pageGunMods, "Infinite Ammo", function() return gunModSettings.infiniteAmmo end, function(v) gunModSettings.infiniteAmmo = v; updateAllWeaponMods() end, 8)
mkToggle(pageGunMods, "Fast Fire", function() return gunModSettings.fastFire end, function(v) gunModSettings.fastFire = v; updateAllWeaponMods() end, 50)
mkSlider(pageGunMods, "Fire Rate", 0.01, 0.2, 0.01, function() return gunModSettings.fireRate end, function(v) gunModSettings.fireRate = v; if gunModSettings.fastFire then updateAllWeaponMods() end end, 92)
mkToggle(pageGunMods, "No Recoil", function() return gunModSettings.noRecoil end, function(v) gunModSettings.noRecoil = v; updateAllWeaponMods() end, 152)
mkToggle(pageGunMods, "All Automatic", function() return gunModSettings.allAuto end, function(v) gunModSettings.allAuto = v; updateAllWeaponMods() end, 194)
mkToggle(pageGunMods, "No Spread", function() return gunModSettings.noSpread end, function(v) gunModSettings.noSpread = v; updateAllWeaponMods() end, 236)
...
guiElements.fov = mkSlider(pageMisc, "Field of View", 70, 120, 1, function() return settings.fov end, function(v) settings.fov = v; Camera.FieldOfView = v end, 194)
...
local rageBtn, rageBtnStroke = mkButton(pageSettings, "Load Rage Config", 254, 282, 260)
local themeTitle = Instance.new("TextLabel", pageSettings); themeTitle.Size=UDim2.new(1,-24,0,20); themeTitle.Position=UDim2.new(0,12,0,300); themeTitle.BackgroundTransparency=1; themeTitle.Font=Enum.Font.GothamBold; themeTitle.Text="Themes"; themeTitle.TextColor3=Color3.new(1,1,1); themeTitle.TextXAlignment=Enum.TextXAlignment.Left; themeTitle.TextSize=16
local themeDarker, themeDarkerStroke = mkButton(pageSettings, "Darker", 330, 12, 125)
local themeBright, themeBrightStroke = mkButton(pageSettings, "Bright", 330, 147, 125)
local themeOcean, themeOceanStroke = mkButton(pageSettings, "Ocean", 330, 282, 125)
local themeAmethyst, themeAmethystStroke = mkButton(pageSettings, "Amethyst", 330, 417, 125)

local function updateGUIFromSettings() for name, el in pairs(guiElements) do if el.Update then el.Update() end; if el.Label and el.Fill then local val = settings[name]; if val ~= nil then el.Label.Text = el.Label.Text:match("(.+):") .. " " .. tostring(val); el.Fill.Size = UDim2.new((val - el.Min) / (el.Max - el.Min), 0, 1, 0) end end end; guiElements.toggleGUIKeybind_button.Button.Text = settings.keybinds.toggleGUI.Name; guiElements.toggleFlyKeybind_button.Button.Text = settings.keybinds.toggleFly.Name; guiElements.aimbotKeyKeybind_button.Button.Text = settings.keybinds.aimbotKey.Name end
...
legitBtn.MouseButton1Click:Connect(function() loadConfig({aimbotEnabled=true, fovCircle=false, aimRadius=80, smoothness=0.5, espEnabled=true, boxESP=true, skelESP=false, healthBar=true, infoESP=true, fov=80, fullBright=false, noclipEnabled=false}) end)
rageBtn.MouseButton1Click:Connect(function() loadConfig({aimbotEnabled=true, fovCircle=true, aimRadius=300, smoothness=1.0, espEnabled=true, boxESP=true, skelESP=true, healthBar=true, infoESP=true, fov=110, fullBright=true, noclipEnabled=true}) end)
...
--// Event-based character cache setup
_G.AimESP_CACHE_CONNECTIONS = {}
local function onPlayerAdded(player)
	table.insert(_G.AimESP_CACHE_CONNECTIONS, player.CharacterAdded:Connect(function(char)
		characterCache[char] = true
	end))
	if player.Character then
		characterCache[player.Character] = true
	end
end
...
table.insert(_G.AimESP_CACHE_CONNECTIONS, Players.PlayerRemoving:Connect(function(player)
	if player.Character and characterCache[player.Character] then
		characterCache[player.Character] = nil
	end
end))